...
func logMessage(level int, msg string, args ...interface{}) {
    if logger == nil {
    ...
    if level >= currentLevel { // 值越小打印得越多
		...
		logger.Println(fmt.Sprintf("[%s:%d] %s", file, line, formattedMsg))
    }
}
...
// 递归格式化结构体信息
// :TODO: 如果发生了指针解引用,是否要在日志中说明下增加指针地址打印
func formatStruct(s interface{}, indent string) string {
    v := reflect.ValueOf(s)
	// 先检查是否是指针，如果是，则解引用
	if v.Kind() == reflect.Ptr {
		v = v.Elem()
...
func PrintStruct(s interface{}, printToStdout bool) string {
